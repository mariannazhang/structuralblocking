...
df.excl.children <- df.data.children %>% 
  summarize(exp_error = sum(exp_error == "yes"),
...
df.data.children <- df.data.children %>%
  filter(exp_error == "no" & 
           interference == "no" &
           insuff_lang == "no") %>%
  select(-exp_error, -interference, -insuff_lang) 
...
df.subj <- df.tidy %>% 
  ...
  group_by(subject, age, age_cat, age_group, context, explanation) %>% 
  summarize(essentialism = mean(response, na.rm = TRUE))

df.subj.children <- df.tidy %>% 
  filter(!str_detect(question, "normative")) %>% 
  filter(age_group == "child") %>% 
  group_by(subject, age, age_cat, context, explanation) %>% 
  summarize(essentialism = mean(response, na.rm = TRUE))

df.subj.adults <- df.tidy %>% 
  filter(!str_detect(question, "normative")) %>% 
  filter(age_group == "adult") %>% 
  group_by(subject, context, explanation) %>% 
  summarize(essentialism = mean(response, na.rm = TRUE))
```
...
df.means <- df.subj %>%
  group_by(age_group, context, explanation) %>% 
  summarize(essentialism = mean(essentialism, na.rm = TRUE),
...
ggplot(df.subj, aes(x = context, y = essentialism, fill = explanation, color = explanation)) +
  ...
  scale_y_continuous(limits = c(0, 1))
ggsave("FYP_contextExpl.png", width = 5)
```
...
fit.compact <- lm(essentialism ~ 1 + context + explanation, data = df.subj.adults)
fit.aug <- lm(essentialism ~ 1 + context * explanation, data = df.subj.adults)
contextExplInt.adults <- anova(fit.compact, fit.aug)
```
