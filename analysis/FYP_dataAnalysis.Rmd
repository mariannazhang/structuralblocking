...
df.data.adults <- bind_rows(df.data.adults.1, df.data.adults.2)
...
ggplot(df.tidy %>%  filter(data_group == "adult", question == "normative_group"), aes(x = context, y = response, fill = explanation, color = explanation)) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2, jitter.height = 0.02, dodge.width = 0.8),
             alpha = 0.3) + 
  stat_summary(fun.data = "mean_cl_boot",
               position = position_dodge(width = 0.8),
